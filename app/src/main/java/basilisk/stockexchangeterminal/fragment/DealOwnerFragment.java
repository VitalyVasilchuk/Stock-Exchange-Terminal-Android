...
public class DealOwnerFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
...
//            prepareList();
    ...
    private void loadDataFromJson() {
        InputStream is = getResources().openRawResource(R.raw.my_deals);
    ...
    private void loadDataFromHTTPServer(String currencyPair) {
        ...
        String ts = df.format(new Date(System.currentTimeMillis() - 7 * (24 * 60 * 60 * 1000)));
        String ts1 = df.format(new Date());

        String param = "out_order_id=" + outOrderId + "&nonce=" + nonce;// + "&ts=" + ts + "&ts1=" + ts1;

        HttpServerApi api = HttpServerApi.Factory.create();
        Call<Deal[]> deals = api.getDealsOwner(
                currencyPair,
                SingletonSession.Instance().getPublicKey(),
                SingletonSession.Instance().getApiSign(param),
                outOrderId,
                nonce);/*,
                ts,
                ts1);*/
